...
_nvd_available: Optional[bool] = None  # None = pas encore testé
...
# ── Seuils CVSS (standard FIRST/NVD) ──────────────────────────────────────────
CVSS_THRESHOLDS = [
...
def query_nvd(product: str, version: str, cache: dict) -> list[dict]:
    ...
    keyword_full = f"{product} {version}".strip() if version else product
    ...
    raw = _nvd_fetch(keyword_full)

    # Si version précise mais peu de résultats, compléter avec product seul
    if version and len(raw) < 3:
        cache_key_base = f"nvd:{product.lower()}"
        if cache_key_base in cache:
            raw_base = cache[cache_key_base]
        else:
            raw_base = _nvd_fetch(product)
            cache[cache_key_base] = raw_base
        # Fusionner en déduplicant
        seen_ids = {e.get("cve", {}).get("id") for e in raw}
        raw += [e for e in raw_base if e.get("cve", {}).get("id") not in seen_ids]
